...
function SetItemProgressState() {
    const { control, fieldTemplates, setValue } = useContext(AddItemPageContext)
    const [autoCompleteValue, setAutoCompleteValue] = useState<any>(""); //autocomplete's current value
    const [templates, setTemplates] = useState<itemProgressState[]>([])

    useEffect(() => {
        if (fieldTemplates && fieldTemplates.states && fieldTemplates.states.length > 0) {
...
export default SetItemProgressState;
