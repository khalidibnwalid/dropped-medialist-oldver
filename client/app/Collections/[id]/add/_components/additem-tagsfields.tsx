...
function AddTagsFields({ tagsData }: { tagsData: itemTag[] }) {
    const { control } = useContext(AddItemPageContext)
    const searchRef = useRef<HTMLInputElement>(null)

    const [usedTags, setUsedTags] = useState<string[]>([]); //current added tags' array
    const [newTags, setNewTags] = useState<string[]>([])

    const [displayedTags, setDisplayedTags] = useState<itemTag[]>(tagsData || [])
    const groupedTags = tagsGroupsSorter(displayedTags)
                ...
                >
                    ...
                    <PopoverContent className="px-4 py-2 rounded-2xl grid">
                        ...
                        <CheckboxGroup
                            ...
                            className="grid grid-cols-1 min-w-60 max-w-80 gap-y-1 overflow-y-auto overflow-x-hidden max-h-96 pr-2 "
                        >
