...
function EditItemBadges() {

    const { control, setValue, getValues, fieldTemplates, itemData } = useContext(EditItemPageContext)

    const templates = fieldTemplates?.badges
    let badgesNum = 0

    itemData.badges?.forEach(() => {
        badgesNum++
        // setValue("badges", itemData.badges)
    })
    // useEffect(() => setValue("badges", itemData.badges), [])
            ...
            <SortableFields
                ...
                {({ data, index, removeField, fieldControl }) => (
                    ...
                    >
                        ...
                        <Controller
                            defaultValue={() => getValues<any>(`badges[${index}]`)?.value}
